...
class WhotGtk(images):
    ...
    def __init__(self):
        ...
        #####################################################################################
        self._played = tk.Label(self._played_cards_frame, image=self._whot.PlayedCards()[len(self._whot.PlayedCards())-1][0])
            ...
            try:
                for i in enumerate(self._labs_refs):
                    if not i[1]:
                        ...
                        #    if not self._found_num and i[1][0][1][1] == 20:
                        #        self._found_20.append([i[0], i[1]])
                        #        self._labs_refs.insert(self._labs_refs.index(i[1]), [])
                        #        self._labs_refs.pop(self._labs_refs.index(i[1]))
                        #        break
